    ...
    <div class="sidebar">
        <el-menu :default-active="onRoutes" class="el-menu-vertical-demo" unique-opened=true router>
            ...
            <!--  <template v-for="(item,index) in menulists">
                <el-menu-item :index="item.linkedUrl" v-if="!item.child">
                    <i class="el-icon-menu"></i>{{item.menuName}}
                </el-menu-item>
                <el-submenu v-if="item.child" :index="item.menuIconId">
                    <template slot="title"><i class="el-icon-menu"></i>{{item.menuName}}</template>
                    <el-menu-item :index="sitem.linkedUrl" v-for="(sitem,sindex) in item.child">{{sitem.menuName}}</el-menu-item>
                </el-submenu>
            </template> -->
        </el-menu>
    </div>
...
export default {
    data() {
        ...
        mounted() {
            //获取菜单数据
            // commonAjax( "base.menuService", "findPcMenu","[]").then(res => {
            //     if (res.code == 200) {
